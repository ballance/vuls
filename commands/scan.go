...
func (p *ScanCmd) SetFlags(f *flag.FlagSet) {
	...
	f.BoolVar(
		...
		"ask-sudo-password",
		false,
		"[Deprecated] THIS OPTION WILL BE REMOVED FOR SECURITY REASON IN A FUTURE RELEASE. Define NOPASSWD in /etc/sudoers on tareget servers and use SSH key-based authentication",
	)
...
func (p *ScanCmd) Execute(_ context.Context, f *flag.FlagSet, _ ...interface{}) subcommands.ExitStatus {
	...
	if p.askSudoPassword {
		logrus.Errorf("[Deprecated] -ask-sudo-password THIS OPTION WILL BE REMOVED FOR SECURITY REASONS IN A FUTURE RELEASE. Define NOPASSWD in /etc/sudoers on tareget servers and use SSH key-based authentication")
		return subcommands.ExitFailure
	}
